...
source $ad_hdl_dir/scripts/ip_lib/adi_xilinx_device_info_enc.tcl
...
proc adi_ip_create {ip_name} {
  ...
  set ip_constr_files ""
  set lib_dirs $ad_hdl_dir/source/ip_library
  if {$ad_hdl_dir ne $ad_phdl_dir} {
    lappend lib_dirs $ad_phdl_dir/source/ip_library
  }
...
proc adi_init_bd_tcl {} {
  ...
  puts $bd_tcl "  for {set x 1} {\$x<=10} {incr x} {"
  puts $bd_tcl "    set linkname \${ip_path}\${parent_dir}scripts/ip_lib/adi_xilinx_device_info_enc.tcl"
  puts $bd_tcl "    if { \[file exists \$linkname\] } {"
  puts $bd_tcl "      source \${ip_path}\${parent_dir}/scripts/ip_lib/adi_xilinx_device_info_enc.tcl"
  puts $bd_tcl "      break"
  puts $bd_tcl "    }"
